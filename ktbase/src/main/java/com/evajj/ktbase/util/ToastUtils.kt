...
object ToastUtils {
    ...
    private var yOffset =
        (64 * Utils.context.getResources().getDisplayMetrics().density + 0.5) as Int
    ...
    private fun show(@StringRes resId: Int, duration: Int) {
        show(Utils.context.getResources().getText(resId).toString(), duration)
    }
    ...
    private fun show(@StringRes resId: Int, duration: Int, vararg args: Any?) {
        show(
            java.lang.String.format(Utils.context.getResources().getString(resId), args),
            duration
        )
    }
