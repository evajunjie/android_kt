 ...
 */
fun byte2MemorySize(byteNum: Long, @MemoryUnit unit: Int): Double {
...
fun byte2FitMemorySize(byteNum: Long): String? {
    return if (byteNum < 0) {
        ...
        java.lang.String.format("%.3fKB", byteNum.toDouble() / KB + 0.0005)
    } else if (byteNum < GB) {
        java.lang.String.format("%.3fMB", byteNum.toDouble() / MB + 0.0005)
    } else {
        java.lang.String.format("%.3fGB", byteNum.toDouble() / GB + 0.0005)
    }
}
...
fun inputStream2String(`is`: InputStream?, charsetName: Charset): String? {
    return if (`is` == null || isSpace(charsetName.toString())) null else try {
...
fun outputStream2String(out: OutputStream?, charsetName: Charset): String? {
    return if (out == null || isSpace(charsetName.toString())) null else try {
...
fun dp2px(dpValue: Float): Int {
    val scale: Float = Utils.context?.resources?.displayMetrics?.density ?: 1f
...
fun sp2px(spValue: Float): Int {
    val fontScale: Float = Utils.context?.resources?.displayMetrics?.scaledDensity ?: 1f
